...
# Этот бот — персональный, под момент-хедж-идею: работает на ОБОИХ
# таймфреймах разом (5m и 15m), без часового (не нужен для этой идеи).
# Через запятую в .env, если захочешь торговать только одним.
_enabled_labels = {s.strip() for s in os.getenv("ENABLED_TIMEFRAMES", "5m,15m").split(",") if s.strip()}
